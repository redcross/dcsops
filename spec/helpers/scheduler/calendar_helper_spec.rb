...
describe Scheduler::CalendarHelper, :type => :helper do
  describe "#render_shift_assignment_info" do
    ...
    let(:shift_time) { double(:shift_time, id: SecureRandom.random_number(100000)) }
    ...
    it "should render a checkbox when person can sign up" do
      out = helper.render_shift_assignment_info(true, person, shift, shift_time, [], date, [], 'daily')
    ...
    it "should not render a checkbox if editable is false" do
      out = helper.render_shift_assignment_info(false, person, shift, shift_time, [], date, [], 'daily')
      expect(out).to match(shift.name + ":")
      expect(out).not_to match("checkbox")
    end
    ...
    it "should not render a checkbox if person is signed up for another shift" do
      ...
      out = helper.render_shift_assignment_info(false, person, shift, shift_time, [assignment], date, [], 'daily')
      expect(out).to match(shift.name + ":")
      expect(out).not_to match("checkbox")
    end
    ...
    it "should render a checkbox when person can un-sign up" do
      out = helper.render_shift_assignment_info(true, person, shift, shift_time, [assignment], date, [], 'daily')
      expect(out).to match(shift.name + ":")
      expect(out).to match("checkbox")
    ...
    it "should render a name when the shift has one person" do
      out = helper.render_shift_assignment_info(false, person, shift, shift_time, [], date, [assignment], 'daily')
      expect(out).to match(shift.name + ":")
    ...
    it "should render a count when the shift has more than one person" do
      out = helper.render_shift_assignment_info(false, person, shift, shift_time, [], date, [assignment, assignment], 'daily')
      expect(out).to match(shift.name + ":")
