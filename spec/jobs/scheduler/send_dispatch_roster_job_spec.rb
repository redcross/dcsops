...
describe Scheduler::SendDispatchRosterJob do
  ...
  describe "#perform" do
    it "should run if force is given" do
      job = Scheduler::SendDispatchRosterJob.new(true)
      job.should_receive :run!
      job.perform
    end

    it "should run if there are shifts needing update" do
      job = Scheduler::SendDispatchRosterJob.new(false)
      job.should_receive :run!
  ...
  describe '#run!' do
    it "should call the mailer" do
      ...
      Scheduler::DirectlineMailer.should_receive(:export).with(Date.current-1, Date.current+60).and_return(delivery)
      subject.run!
    end
  end
