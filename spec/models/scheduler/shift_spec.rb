...
describe Scheduler::Shift do
  ...
  describe "can_sign_up_on_day" do
    it "should be true today" do
      shift.can_sign_up_on_day(date, shift_group).should be_true
    end

    it "should be true tomorrow" do
      shift.can_sign_up_on_day(date+1, shift_group).should be_true
    end

    it "should be false yesterday" do
      shift.can_sign_up_on_day(date-1, shift_group).should be_false
    end
    ...
    it "should be false if the shift is not active" do
      ...
      shift.can_sign_up_on_day(date, shift_group).should be_false
    end
    ...
    it "should be false if max_advance_signup is too soon" do
      ...
      shift.can_sign_up_on_day(date+20, shift_group).should be_false
    end

    describe "with signups" do
      before(:each) do
        FactoryGirl.create :shift_assignment, person: person, shift: shift, date: date, shift_group: shift_group
      end
  ...
  describe "can_remove_on_day" do
    it "should be true today" do
      shift.can_remove_on_day(date, shift_group).should be_true
    end

    it "should be true tomorrow" do
      shift.can_remove_on_day(date+1, shift_group).should be_true
    end

    it "should be false yesterday" do
      shift.can_remove_on_day(date-1, shift_group).should be_false
    end

    it "should be false if the shift has been frozen before" do
      shift.update_attribute :signups_frozen_before, date+1
      shift.can_remove_on_day(date, shift_group).should be_false
    end
  ...
  describe "active_on_day?" do
    it "should be false if the shift has not begun yet" do
      ...
      shift.active_on_day?(date, shift_group).should be_false
    end
