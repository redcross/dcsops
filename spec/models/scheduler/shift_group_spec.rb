...
describe Scheduler::ShiftGroup, :type => :model do
  ...
  describe "#current_groups" do
    it "should return current daily groups" do
      ...
      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift1])
      expect(arr.first.start_date).to eq(@chapter.time_zone.today)

      Delorean.time_travel_to@chapter.time_zone.now.change hour: 12

      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift2])
      expect(arr.first.start_date).to be_a(Date)
      expect(arr.first.start_date).to eq(@chapter.time_zone.today)

      Delorean.time_travel_to@chapter.time_zone.now.change hour: 5

      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([])

      Delorean.time_travel_to @chapter.time_zone.now.change hour: 3

      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift2])
      expect(arr.first.start_date).to eq(@chapter.time_zone.today.yesterday)
    ...
    it "should not return a daily group which is not active on that day" do
      ...
      expect(Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([])

      Delorean.time_travel_to 'wednesday 8am'

      expect(Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift1])
    end
    ...
    it "should return current weekly groups" do
      ...
      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift3])
...
#
      ...
      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift1])
      expect(arr.first.start_date).to eq @chapter.time_zone.today.at_beginning_of_week.next_week
    ...
    it "should return current monthly groups" do
      ...
      expect(arr = Scheduler::ShiftGroup.current_groups_for_chapter(@chapter)).to match_array([@shift1])
      expect(arr.first.start_date).to be_a(Date)
      expect(arr.first.start_date).to eq @chapter.time_zone.today.at_beginning_of_month
    end
