module Incidents::GoInstantHelper
  ...
  def goinstant_setup chapter, incident=nil
    return "" unless goinstant_app_url.present? 
    ...
    js = <<-JS
      $(function() {
        if (window.goinstantConnectionPromise) {
          window.incidentInstantController = new IncidentInstantController(window.goinstantConnectionPromise);
          #{incident_setup}
