class Incidents::ResponderMessageService
  ...
  def open_assignment_for_person(person)
    Incidents::ResponderAssignment.joins(:incident).readonly(false).where(incidents_incidents: { status: 'open' }).was_available.for_person(person).order(created_at: :desc).first
  end

  def open_recruitment_for_person(person)
    Incidents::ResponderRecruitment.joins(:incident).readonly(false).where(incidents_incidents: { status: 'open' }).for_person(person).order(created_at: :desc).first
  end
